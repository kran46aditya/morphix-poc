...
def read_with_spark(mongo_uri: str, database: str, collection: str, query: Optional[Dict[str, Any]] = None, limit: int = 10) -> Dict[str, Any]:
    ...
    """
    # Build a fully qualified collection uri for the mongo-spark connector
    uri = mongo_uri
    # The connector expects the namespace via database.collection in the 'uri' or options; we'll pass via read options
    ...
    try:
        ...
        if query:
            # The connector accepts pipeline/filters; keep it simple: pass as pipeline
            reader = reader.option("pipeline", json.dumps([{"$match": query}]))

        df = reader.load()
        # collect a few rows as dicts - careful with large objects
